...
Lb0=3.0176; Lb1=3.0121; Le=1.3953;

*Shift in the error variance with no shifts in the regression parameters;
shift=0;
do gam=1.0 to 3.0 by 0.2 ;
shift=shift+1;
Sige_shift=gam*stdev;
beta0 = 0.00;
delta = 0.00;
...
do i=1 to 1000000000 until (signal=1); 
           ...
           Vz = (lambda*(1-(1-lambda)**(2*i)))/(2-lambda);
...
results=rlvec;
	name1={"1.00" "1.20" "1.40" "1.60" "1.80" "2.00" "2.20" "2.40" "2.60" "2.80" "3.00"};	
...
print 'EWMA3 ERROR VARIANCE TIME-VARYING';
print sim n lambda Lb0 Lb1 Le delta beta0;
